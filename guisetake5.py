...
###############################
###############################
#GUISE - Die symbolische Integrator in Python 
#Um die Integration auf Steroiden Get Used 
# Lynch Envy Grundliche AI Labs (L.E.TH.A.L) 
# GUISE DERZEIT ahnlich wie ein Heiliger.
#Cleanesch Verzhion
...
for i in range(counter):
    if dict[i].level == 2:
        aux.extend(dict[i].val)
    if dict[i].level == 1 and dict[i].opstat == 1:
        lvstat = i
print aux
print lvstat

for i in range(0,lvstat):
    output.extend(dict[i].val)

for i in range(lvstat,counter):
    if dict[i].level != 1:
